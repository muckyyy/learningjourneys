...
echo ""
echo "=== APACHE PROXY MODULES CONFIGURATION ==="

# Check if Apache proxy modules are available
echo "Checking available Apache proxy modules..."
if [ -d "/usr/lib64/httpd/modules" ]; then
    echo "Available proxy modules:"
    ls /usr/lib64/httpd/modules/ | grep proxy | head -10
    echo ""
else
    echo "⚠ Apache modules directory not found"
fi

# Enable required Apache proxy modules for streaming
echo "Configuring Apache proxy modules for streaming and WebSocket support..."

# First, ensure standard proxy modules configuration exists
STANDARD_PROXY_CONF="/etc/httpd/conf.modules.d/00-proxy.conf" 

# Check if the standard proxy configuration file exists, if not create it
if [ ! -f "$STANDARD_PROXY_CONF" ]; then
    echo "Creating standard Apache proxy modules configuration..."
    cat > "$STANDARD_PROXY_CONF" << 'EOF'
# Proxy modules for Apache HTTP Server
# This file configures the proxy modules for the Apache HTTP Server
...
EOF
    echo "✓ Standard proxy modules configuration created"
else
    echo "✓ Standard proxy configuration exists at: $STANDARD_PROXY_CONF"
fi

# Ensure critical modules are enabled by checking and enabling them individually
REQUIRED_MODULES=(
...
echo "Ensuring required proxy modules are enabled:"
for MODULE_DEF in "${REQUIRED_MODULES[@]}"; do
    MODULE_NAME=$(echo "$MODULE_DEF" | cut -d' ' -f1)
    MODULE_PATH=$(echo "$MODULE_DEF" | cut -d' ' -f2-)
    
    # Check if module is already in the config file
    if grep -q "LoadModule $MODULE_NAME" "$STANDARD_PROXY_CONF"; then
        # Enable it if it's commented out
        if grep -q "^#.*LoadModule $MODULE_NAME" "$STANDARD_PROXY_CONF"; then
            sed -i "s/^#.*LoadModule $MODULE_NAME.*/LoadModule $MODULE_DEF/" "$STANDARD_PROXY_CONF"
            echo "✓ Enabled $MODULE_NAME (was commented)"
        else
            echo "✓ $MODULE_NAME already enabled"
        fi
    else
        # Add the module if it's missing
        echo "LoadModule $MODULE_DEF" >> "$STANDARD_PROXY_CONF"
        echo "✓ Added $MODULE_NAME to configuration"
    fi
done

# Ensure we don't have any custom proxy configs that might conflict
CUSTOM_PROXY_CONF="/etc/httpd/conf.modules.d/00-proxy.conf"
if [ -f "$CUSTOM_PROXY_CONF" ] && [ "$CUSTOM_PROXY_CONF" != "$STANDARD_PROXY_CONF" ]; then
...
echo ""
echo "=== APACHE VHOST CONFIGURATION DEPLOYMENT ==="

# Deploy the main VirtualHost configuration (ALWAYS replace on deployment)
MAIN_VHOST_CONF="/etc/httpd/conf.d/learningjourneys.conf"
SOURCE_VHOST_CONF="$APP_DIR/config/apache/learningjourneys.conf"

if [ -f "$SOURCE_VHOST_CONF" ]; then
    echo "✓ Found VirtualHost configuration source: $SOURCE_VHOST_CONF"
    
    # Always create backup of existing config before replacement
    if [ -f "$MAIN_VHOST_CONF" ]; then
        cp "$MAIN_VHOST_CONF" "${MAIN_VHOST_CONF}.backup.$(date +%Y%m%d_%H%M%S)"
        echo "✓ Backup created for existing VirtualHost configuration"
        echo "ⓘ Replacing existing configuration with latest version from repository"
    else
        echo "ⓘ No existing configuration found, deploying new configuration"
    fi
    
    # Always copy/replace the configuration from repository
    cp "$SOURCE_VHOST_CONF" "$MAIN_VHOST_CONF"
    echo "✓ VirtualHost configuration deployed/updated: $MAIN_VHOST_CONF"
    
    # Set proper permissions
    chown root:root "$MAIN_VHOST_CONF"
    chmod 644 "$MAIN_VHOST_CONF"
    echo "✓ Permissions set for VirtualHost configuration"
    
    # Show diff if backup exists (to see what changed)
    LATEST_BACKUP=$(ls -t "${MAIN_VHOST_CONF}.backup."* 2>/dev/null | head -1)
    if [ -n "$LATEST_BACKUP" ] && [ -f "$LATEST_BACKUP" ]; then
        echo ""
        echo "Configuration changes in this deployment:"
        if diff -u "$LATEST_BACKUP" "$MAIN_VHOST_CONF" > /tmp/vhost_diff.txt 2>/dev/null; then
            echo "  ⓘ No changes detected in VirtualHost configuration"
        else
            echo "  📝 Changes detected:"
            head -20 /tmp/vhost_diff.txt | sed 's/^/    /'
            if [ $(wc -l < /tmp/vhost_diff.txt) -gt 20 ]; then
                echo "    ... (showing first 20 lines, full diff in /tmp/vhost_diff.txt)"
            fi
        fi
        rm -f /tmp/vhost_diff.txt
        echo ""
    fi
    
    # Verify the configuration
    echo "Verifying VirtualHost configuration syntax..."
    if httpd -t 2>/dev/null; then
        echo "✓ VirtualHost configuration syntax is valid"
        
        # Check for key features
        echo "Checking VirtualHost configuration features:"
        if grep -q "flushpackets=on" "$MAIN_VHOST_CONF"; then
            echo "  ✓ PHP-FPM streaming enabled (flushpackets=on)"
        else
            echo "  ⚠ PHP-FPM streaming not configured"
        fi
        
        if grep -q "SetEnv no-gzip" "$MAIN_VHOST_CONF"; then
            echo "  ✓ Compression disabled for streaming routes"
        else
            echo "  ⚠ Compression settings not found"
        fi
        
        if grep -q "ProxyPass.*ws://" "$MAIN_VHOST_CONF"; then
            echo "  ✓ WebSocket proxy configured"
        else
            echo "  ⚠ WebSocket proxy not found"
        fi
        
        if grep -q "SSLEngine on" "$MAIN_VHOST_CONF"; then
            echo "  ✓ SSL configuration present"
        else
            echo "  ⚠ SSL configuration not found"
        fi
        
        echo "✓ VirtualHost configuration successfully updated and verified"
        
    else
        echo "✗ VirtualHost configuration syntax error"
        echo "Configuration test output:"
        httpd -t
        echo ""
        echo "🔄 Restoring previous working configuration..."
        if [ -n "$LATEST_BACKUP" ] && [ -f "$LATEST_BACKUP" ]; then
            cp "$LATEST_BACKUP" "$MAIN_VHOST_CONF"
            echo "✓ Restored backup: $LATEST_BACKUP"
            
            # Test the restored configuration
            if httpd -t 2>/dev/null; then
                echo "✓ Restored configuration syntax is valid"
            else
                echo "✗ Even backup configuration has syntax errors!"
                echo "❌ DEPLOYMENT FAILURE: Invalid Apache configuration"
                exit 1
            fi
        else
            echo "❌ No backup available to restore!"
            echo "❌ DEPLOYMENT FAILURE: Invalid Apache configuration and no backup"
            exit 1
        fi
    fi
    
else
    echo "❌ VirtualHost configuration source not found: $SOURCE_VHOST_CONF"
    echo "❌ DEPLOYMENT FAILURE: Required Apache configuration missing from repository"
    echo "Expected location: config/apache/learningjourneys.conf in application root"
    exit 1
fi

echo ""
echo "=== STREAMING-SPECIFIC APACHE CONFIGURATION ==="

# Check if custom learningjourneys.conf exists and is properly configured
MAIN_VHOST_CONF="/etc/httpd/conf.d/learningjourneys.conf"
CUSTOM_STREAMING_CONF="/etc/httpd/conf.d/laravel-streaming-supplement.conf"

if [ -f "$MAIN_VHOST_CONF" ]; then
    echo "✓ Found main VirtualHost configuration: $MAIN_VHOST_CONF"
    
    # Verify streaming settings exist in main config
    if grep -q "LocationMatch.*api/chat" "$MAIN_VHOST_CONF"; then
        echo "✓ Streaming configuration found in main VirtualHost"
        echo "ⓘ Skipping creation of duplicate streaming config"
        
        # Just verify essential settings are present
        echo "Verifying streaming settings in main configuration:"
        if grep -q "no-gzip" "$MAIN_VHOST_CONF"; then
            echo "  ✓ Compression disabled for streaming"
        else
            echo "  ⚠ Compression settings not found"
        fi
        
        if grep -q "flushpackets=on" "$MAIN_VHOST_CONF"; then
            echo "  ✓ PHP-FPM flush packets enabled"
        else
            echo "  ⚠ PHP-FPM flush packets not configured"
        fi
        
        if grep -q "mod_proxy" "$MAIN_VHOST_CONF" && grep -q "/app/" "$MAIN_VHOST_CONF"; then
            echo "  ✓ WebSocket proxy configuration found"
        else
            echo "  ⚠ WebSocket proxy not configured in main config"
        fi
        
    else
        echo "⚠ No streaming configuration found in main VirtualHost"
        echo "Creating supplemental streaming configuration..."
        
        # Create minimal supplement to existing config
        cat > "$CUSTOM_STREAMING_CONF" << 'SUPPLEMENT_EOF'
# Supplemental streaming configuration for Laravel
# This supplements the main VirtualHost configuration
# Generated by deployment script

# Additional streaming optimizations if not present in main config
<IfModule mod_headers.c>
    # Global streaming headers for API endpoints
    <LocationMatch "^/api/(chat|audio)/.*">
        # Ensure no buffering headers
        Header always set X-Accel-Buffering "no"
        Header always set X-Output-Buffering "off"
...
# Ensure compression is disabled for streaming endpoints
<IfModule mod_deflate.c>
    <LocationMatch "^/api/(chat|audio)/.*">
        SetEnvIf Request_URI "^/api/(chat|audio)/.*" no-gzip=1
        SetEnvIf Request_URI "^/api/(chat|audio)/.*" no-brotli=1
    </LocationMatch>
</IfModule>
SUPPLEMENT_EOF
        echo "✓ Supplemental streaming configuration created"
    fi
    
else
    echo "⚠ Main VirtualHost configuration not found: $MAIN_VHOST_CONF"
    echo "Creating complete Laravel streaming configuration: $CUSTOM_STREAMING_CONF"

    # Create complete configuration since main config doesn't exist
    cat > "$CUSTOM_STREAMING_CONF" << 'COMPLETE_EOF'
# Complete Laravel streaming and WebSocket configuration
# Generated by deployment script

# Disable buffering for streaming responses
<LocationMatch "^/api/chat/(start|submit)">
    # Disable compression for streaming endpoints
    SetEnv no-gzip 1
    SetEnv no-brotli 1
    RemoveOutputFilter DEFLATE
    RemoveOutputFilter BROTLI
    ...
    Header set X-Output-Buffering "off"
    Header set Transfer-Encoding "chunked"
    Header set Connection "keep-alive"
</LocationMatch>

# WebSocket proxy configuration for Reverb (if not in main config)
<IfModule mod_proxy.c>
    ProxyRequests Off
    ProxyPreserveHost On
...
COMPLETE_EOF
    echo "✓ Complete Laravel streaming configuration created"
fi

# Test the new streaming configuration
if httpd -t 2>/dev/null; then
    echo "✓ Apache configuration syntax is valid"
    
    echo "Reloading Apache to apply configuration changes..."
    if systemctl reload httpd; then
        echo "✓ Apache configuration reloaded successfully"
    else
        echo "⚠ Apache reload failed, attempting restart..."
        systemctl restart httpd && echo "✓ Apache restarted" || echo "✗ Apache restart failed"
    fi
else
    echo "✗ Apache configuration syntax error"
    echo "Configuration test output:"
    httpd -t
    echo "Removing any problematic supplemental configuration..."
    rm -f "$CUSTOM_STREAMING_CONF"
    echo "ⓘ Supplemental configuration removed due to syntax errors"
fi
