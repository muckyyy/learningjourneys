...
if [ -f "$PROXY_CONF" ]; then
    echo "✓ Found Apache proxy configuration: $PROXY_CONF"
    
    # Create backup of existing configuration
    cp "$PROXY_CONF" "${PROXY_CONF}.backup.$(date +%Y%m%d_%H%M%S)"
    echo "✓ Backup created: ${PROXY_CONF}.backup.$(date +%Y%m%d_%H%M%S)"
    
    # Required modules for streaming and WebSocket support
    REQUIRED_MODULES=(
        "mod_proxy.so"
        "mod_proxy_http.so"
        "mod_proxy_fcgi.so"
    ...
    echo "Enabling required Apache modules for streaming:"
    
    # Create new proxy configuration
    cat > "$PROXY_CONF" << 'PROXY_EOF'
# Proxy modules configuration for Laravel streaming and WebSocket support
# Generated by deployment script

# Core proxy module (required for all proxy functionality)
LoadModule proxy_module modules/mod_proxy.so

# HTTP proxy (for standard web requests and streaming)
LoadModule proxy_http_module modules/mod_proxy_http.so

# FastCGI/PHP-FPM proxy (for PHP processing)
LoadModule proxy_fcgi_module modules/mod_proxy_fcgi.so

# Load balancer (for multiple backend servers if needed)
